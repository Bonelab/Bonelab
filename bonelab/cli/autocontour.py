...
def convert_aim_to_density(img:vtk.vtkImageData,m:float,b:float):
    mult = vtk.vtkImageMathematics()
    mult.SetOperationToMultiplyByK()
    mult.SetConstantK(m)
    mult.SetInputData(img)
...
def autocontour_buie(img, thresh1, thresh2, viz=False) -> vtk.vtkImageData:
    ...
    if viz:
        s2_threshold.Update()
        show_progress(s2_threshold.GetOutput(),'step 2: threshold')
    ...
    if viz:
        s3_median.Update()
        show_progress(s3_median.GetOutput(),'step 3: median')
    ...
    if viz:
        s4_dilate.Update()
        show_progress(s4_dilate.GetOutput(),'step 4: dilate')
    ...
    if viz:
        s5a_invert.Update()
        show_progress(s5a_invert.GetOutput(),'step 5a: connectivity')
    ...
    if viz:
        s5b_connectivity.Update()
        show_progress(s5b_connectivity.GetOutput(),'step 5b: connectivity')
    ...
    if viz:
        s5c_invert.Update()
        show_progress(s5c_invert.GetOutput(),'step 5c: connectivity')
    ...
    if viz:
        s6_erode.Update()
        show_progress(s6_erode.GetOutput(),'step 6: erode')
    ...
    if viz:
        s7_threshold.Update()
        show_progress(s7_threshold.GetOutput(),'step 7: threshold')
    ...
    if viz:
        s8a_mask.Update()
        show_progress(s8a_mask.GetOutput(),'step 8a: mask')
    ...
    if viz:
        s8b_invert.Update()
        show_progress(s8b_invert.GetOutput(),'step 8b: mask')
    ...
    if viz:
        s9_dilate.Update()
        show_progress(s9_dilate.GetOutput(),'step 9: dilate')
    ...
    if viz:
        s10a_connectivity.Update()
        show_progress(s10a_connectivity.GetOutput(),'step 10a: connectivity')
    ...
    if viz:
        s10b_convert.Update()
        show_progress(s10b_convert.GetOutput(),'step 10b: connectivity')
    ...
    if viz:
        s11_erode.Update()
        show_progress(s11_erode.GetOutput(),'step 11: erode')
    ...
    if viz:
        s12_gauss.Update()
        show_progress(s12_gauss.GetOutput(),'step 12: gaussian smooth')
    ...
    if viz:
        s13_threshold.Update()
        show_progress(s13_threshold.GetOutput(),'step 13: threshold')
...
def main():
    ...
    SOFTWARE = 'Buie-Burghardt-Scanco Autoconour VTK Implemention'
    VERSION = 0.1
