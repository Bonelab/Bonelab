...
while (sample_number <= end):
    ...
    while (sample_number <= end):
        if sample_number < 10:
            filename_original_bone = dirname_original1 + str(sample_number) + "_R_F_THICK_LAT_Morph_roi.mha"
            filename_original_cart = dirname_original1 + str(sample_number) + "_R_FC_THICK_LAT_Morph_roi.mha"
        else:
            filename_original_bone = dirname_original2 + str(sample_number) + "_R_F_THICK_LAT_Morph_roi.mha"
            filename_original_cart = dirname_original2 + str(sample_number) + "_R_FC_THICK_LAT_Morph_roi.mha"
        ...
        mean_cent_bone = np.mean(cent_mean_bone)/1000 #Divide because we multiplied thickness values by 1000 before morphing
