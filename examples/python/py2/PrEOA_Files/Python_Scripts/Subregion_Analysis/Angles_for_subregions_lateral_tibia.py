...
angle_increment = 180 #enter in degrees, this is the angle from the line between the centre of mass which divides the subregions if more than one subregion was needed
angle_from_shift = 0
filename = "/Users/jbhatla/Documents/UofC/PrEOA/Analysis/Morphing/Thickness_Map_Scaled/PREOA_005_L_XCT_T_LAT_roi.mha"
...
length_lat = len(x_points_lat) #calculate number of points
centroid_x_lat = sum(x_points_lat)/length_lat #calculate centroid
...
for i in range(npoints):
        ...
        print angle ######## This is the angle to each point you will need #######
...
# Adjust this to create each mask desired
for x in range (0, (x_dim)):
    for y in range (0, (y_dim)):
        value = final_image[[x], [y]]
        if value ==1 and y > (m1*x + b1):
            anterior_mask[[x], [y]] = 1
        else:
            anterior_mask[[x], [y]] = 0
np.savetxt("/Users/jbhatla/Documents/UofC/PrEOA/Analysis/Subregions/posterior_mask_lat_tib.txt", lateral_mask)

plot = plt.figure()
plt.imshow(anterior_mask,cmap= "nipy_spectral", vmin=-0, vmax=3)
cbar = plt.colorbar()
plt.show()
