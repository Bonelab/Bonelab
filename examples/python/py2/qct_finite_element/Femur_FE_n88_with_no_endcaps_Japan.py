...
if (len(sys.argv) != 12):
  print "Usage: vtkpython Femur_FE example.conf NORM_001.AIM cortMask.vtk trabmask.vtk 1.22 -3.34 33.7 15 10 11 175"
  sys.exit(1)
...
angleOther = sys.argv[10]
shaftLength = sys.argv[11]
...
transform.RotateX(float(angleOther))
...
finalImage.SetInput(image2)
finalImage.ThresholdByLower(252)
finalImage.SetOutValue(252)
finalImage.Update()
...
writer = vtkn88.vtkn88AIMWriter()
writer.SetInput (finalImage.GetOutput())  # REMEMBER TO CHANGE THIS WHEN ADDING SHAFT CAP
...
model.AddNodeSet (visibleNodesIds, "SHAFT VISIBLE NODES1")
#Log ("Found %d visible nodes." % visibleNodesIds.GetNumberOfTuples())

# We're going to need these points as vtkPolyData, for one to write them
# out, but also to further filter based on geometry, so let's grab
# them now as vtkPolyData
visibleBoneVertices = model.DataSetFromSelection("SHAFT VISIBLE NODES1")
...
Log ("Limiting shaft to box bounds:",
     ("%.4f" + " %.4f"*5) % boxBounds)
box = vtk.vtkBox()
box.SetBounds (boxBounds)
...
transform.Translate(-visibleNodesBounds[0], -visibleNodesBounds[2], -visibleNodesBounds[4])
transform.RotateY(float(40))
transform.Translate(visibleNodesBounds[0], visibleNodesBounds[2], visibleNodesBounds[4])


Log ("Prior to finding NECK nodes:",
     ("%.4f" + " %.4f"*5) % visibleNodesBounds)


midPointNeck1 = ((visibleNodesBounds[5]+visibleNodesBounds[4])/2-40)
midPointNeck2 = ((visibleNodesBounds[5]+visibleNodesBounds[4])/2-30)
