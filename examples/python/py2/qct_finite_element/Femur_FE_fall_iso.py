...
gauss.SetInput(reslice.GetOutput())
...
print "After calibration1, it has a range of %.3f to %.3f" % (minImage, maxImage)
...
#!!! check with Eric what is fixed 0 or 1
model.SetConstraintFixed ("SHAFT VISIBLE NODES",  model.NodeIdsFromSelection("SHAFT VISIBLE NODES").GetNumberOfTuples(),0,0,0,"SHAFT SURFACE CONSTRAINT")

# Now get these nodes (with point coordinates) and save them to a file
# so that we can examine them with ParaView, etc...
nodesFileShaft = os.path.splitext(inputAimFile)[0] + "_shaft_nodes_FAL_ISO.vtu"
...
#!!! check with Eric what is fixed 0 or 1
#model.SetConstraintFixed ("SHAFT END NODES",  model.NodeIdsFromSelection("SHAFT END NODES").GetNumberOfTuples(),0,0,0,"SHAFT SURFACE CONSTRAINT")
...
model.SetConstraintFixed ("BONE BOTTOM VISIBLE NODES",  model.NodeIdsFromSelection("BONE BOTTOM VISIBLE NODES").GetNumberOfTuples(),1,0,1,"TROCHANTER SURFACE CONSTRAINT")
...
boxBounds = (visibleNodesBounds[0],
		...
		visibleNodesBounds[2],
		visibleNodesBounds[3],
		visibleNodesBounds[5]- ((visibleNodesBounds[5]-visibleNodesBounds[4])/4),
		visibleNodesBounds[5]) 
...
model.SetConstraint (
	...
	1,
    1,
    model.NodeIdsFromSelection("BONE TOP VISIBLE NODES").GetNumberOfTuples(),
    headDisplacement,
