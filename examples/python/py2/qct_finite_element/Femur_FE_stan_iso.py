...
if (len(sys.argv) != 5):
  print "Usage: vtkpython Femur_FE example.conf NORM_001.AIM 1.22 -3.34"
  sys.exit(1)
...
outputFaimFile = os.path.splitext(inputAimFile)[0] + "_STA_ISO.inp"
...
outputAimFile = os.path.splitext(inputAimFile)[0] + "_CAL_STA_ISO.AIM"
...
# -------------------------------------------------------------------------
# Filter with gaussian and rescale to 1.5 mm isotropic voxels
...
reslice.SetInput(gauss.GetOutput())
#reslice.SetInputConnection(reader.GetOutputPort()) #use these if no gaussian used
...
outputModelFile = os.path.splitext(inputAimFile)[0] + "_model_STA_ISO.vtu"
...
generator = vtkski.vtkskiMeshGenerator()
...
nodesFileShaft = os.path.splitext(inputAimFile)[0] + "_shaft_nodes_STA_ISO.vtu"
...
boxBounds = (visibleNodesBounds[0],
		...
		visibleNodesBounds[3],
		visibleNodesBounds[4],
		visibleNodesBounds[4] + headConstraintMaxDepth) 
...
# Save to file so we can examine in ParaView, etc...
depthFilteredBoneNodesFile = os.path.splitext(inputAimFile)[0] + "_nodes_head_STA_ISO.vtu"
writer = vtk.vtkXMLUnstructuredGridWriter()
